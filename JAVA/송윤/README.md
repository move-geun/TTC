...
- Heap Area
  ...
  : 객체와 배열이 생성되는 영역
    ...
    만일 참조하는 변수나 필드가 없다면 의미 없는 객체가 되기 때문에 JVM이 쓰레기로 취급하고 Garbage Collector를 실행시켜 자동으로 제거.
...
- new 연산자로 배열 생성
  ...
  ```java
  int[] scores = new int[30]; // 배열 크기 = 30, 모두 0으로 초기화 
...
: 자바는 1차원 배열이 서로 연결된 구조로 다차원 배열을 구현하기 때문에 계단식 구조를 가질 수 있다. 
